...
import { PermissionContext } from '../component/JFPermission/JFPermission';
import { comparePermission } from '../common/utils';
// import { JFHeaderItem } from './component/JFHeaderItem/JFHeaderItem';
import { JFStatementIcon } from '../component/JFIcon/JFIcon';
import { JFAvatar, JFAvatarItem } from './component/JFAvatar/JFAvatar';

const { SubMenu } = Menu;
const { Header, Sider } = Layout;
const Icons = {
    JFStatement: <JFStatementIcon />,
};
...
class SwitchLayout extends Component {
    ...
    // 根据当前path改变menu状态
    changeMenuStatus = () => {
        const curPath = this.props.history.location.pathname.substring(this.props.history.location.pathname.indexOf('ta') - 1);
        const curMenu = curPath.split('/').slice(2);
        const keys = this.getCurrentSelectedKeys(curMenu) || [];
        this.setState({
            selectedKeys: keys,
            openKeys: keys,
        });
